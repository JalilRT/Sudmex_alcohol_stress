...
cat << EOF > ${path}/code/LME_SB/models/M4H_${rois}.sh
#!/bin/bash
#SBATCH --time=03:30:00
#SBATCH --nodes=1
#SBATCH --ntasks=80
...
singularity exec -B /scratch:/scratch ${container} \
 3dLMEr -prefix /scratch/m/mchakrav/jrasgado/sudmex_alcohol_rat/analysis/fmri/Seed_based/models/M4H_${rois}.nii.gz -jobs 1 \
	-mask /scratch/m/mchakrav/jrasgado/sudmex_alcohol_rat/analysis/fmri/Seed_based/mask_registered.nii.gz \
        ...
        -qVars 'Age' \
        -gltCode interaction 'Intake : 1*High -1*Ctrl Sex : 1*male -1*female' \
        -dataTable @/scratch/m/mchakrav/jrasgado/sudmex_alcohol_rat/analysis/fmri/code/LME_SB/${rois}.txt

EOF

sbatch ${path}/code/LME_SB/models/M4H_${rois}.sh
