...
phen$Group <- factor(phen$Group, levels = c("Ctrl","Alc","AlcStr","Str"))
# Network matrices
net <- readRDS(file.path(data_dir,"fc_matrices.rds"))
net_dim <- dim(net)

# Models
# Modelo 3. Group-Age-Sex interaction.
...
tic <- Sys.time()
fit_nbr_mod3_gt_thr05 <- nbr_lme(net = net,
                   nnodes = net_dim[1],
                   idata = phen,
                   mod = mod,
                   rdm = rdm,
...
tic <- Sys.time()
fit_nbr_mod3_lt_thr05 <- nbr_lme(net = net,
                                 nnodes = net_dim[1],
                                 idata = phen,
                                 mod = mod,
                                 rdm = rdm,
...
# Alternative greater than
tic <- Sys.time()
fit_nbr_mod3_gt_thr01 <- nbr_lme(net = net,
                                 nnodes = net_dim[1],
                                 idata = phen,
                                 mod = mod,
                                 rdm = rdm,
...
# Alternative lower than
tic <- Sys.time()
fit_nbr_mod3_lt_thr01 <- nbr_lme(net = net,
                                 nnodes = net_dim[1],
                                 idata = phen,
                                 mod = mod,
                                 rdm = rdm,
...
# Alternative greater than
tic <- Sys.time()
fit_nbr_mod3_gt_thr10 <- nbr_lme(net = net,
                                 nnodes = net_dim[1],
                                 idata = phen,
                                 mod = mod,
                                 rdm = rdm,
...
# Alternative lower than
tic <- Sys.time()
fit_nbr_mod3_lt_thr10 <- nbr_lme(net = net,
                                 nnodes = net_dim[1],
                                 idata = phen,
                                 mod = mod,
                                 rdm = rdm,
...
# Save results
save(fit_nbr_mod3_gt_thr10,
     fit_nbr_mod3_lt_thr10,
     fit_nbr_mod3_gt_thr05,
     fit_nbr_mod3_lt_thr05,
     fit_nbr_mod3_gt_thr01,
     fit_nbr_mod3_lt_thr01,
     file = file.path(out_dir,"fit_nbr_mod3_thrs10_05_01.Rdata"))
