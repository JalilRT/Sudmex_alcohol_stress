...
Morph_metrics_CPu_features <- read_csv("Data/Inmunofluorescence/Morph_metrics_CPu_features.csv")
...
u2 <- umap_transform(Morph_metrics_CPu_features[,-c(1:4)] %>% dplyr::select(metrics_umap$x), u1)
...
CPu_phenotypes <- df %>% add_column(Predictions.from.UMAP = clusters,
...
Morph_metrics_CPu_phenotype <- Morph_metrics_CPu_features %>% add_column(CPu_phenotypes %>% select(-Morphology))

cpu_confusion_matrix <- table(Morph_metrics_CPu_phenotype$Group,Morph_metrics_CPu_phenotype$Predictions.from.UMAP) %>% as_tibble(.name_repair = "minimal") %>% set_colnames(c("Group","cluster","n"))

cpu_confusion_matrix %>% mutate(percentage = n/nrow(CPu_phenotypes) * 100) %>% write_csv("Data/Inmunofluorescence/clusters_CPu.csv")

# Plot cpu matrix ---------------------------------------------------------

classes <- c("Round", "Inflamed_Ameboid", "Fried_egg", "Inflamed_Fried_egg", "Hypertrophic", "Inflamed_Hypertrophic", "Bipolar", "Not annotated")

cpu_confusion_matrix %>% 
